...
standard_041_1hr = sumBelowTime(standard_041,1,0,0)
standard_041_3hr = sumBelowTime(standard_041,3,0,0)
standard_041_6hr = sumBelowTime(standard_041,6,0,0)
standard_041_12hr = sumBelowTime(standard_041,12,0,0)
...
DETECTR_042 <- read.csv("../tidy_data/DETECTR_042_tidy_metadata.csv")
standard_041 <- subset(DETECTR_042, DETECTR_042$sample_set == "UMP_standard" & !(is.na(DETECTR_042$time)) & !(is.na(DETECTR_042$rfu_value)))
standard_041$standard_conc <- as.numeric(as.character(standard_041$standard_conc))
...
standard_041_1hr = sumBelowTime(standard_041,0,50,0)
standard_041_3hr = sumBelowTime(standard_041,3,0,0)
standard_041_6hr = sumBelowTime(standard_041,6,0,0)
standard_041_12hr = sumBelowTime(standard_041,12,0,0)
ggplotRegression(lm(standard_041_0hr_single_point$rfu_value~standard_041_0hr_single_point$standard_conc))
...
### Conclusion 
Low concentrations actually look decent on this batch too. 0 hr timepoint looks worse than 1 hr timepoint, as shown in DETECTR 040. Summing doesn't seem to help. I would trust 0.15uM and above.


```{r echo=FALSE}

DETECTR_042 <- read.csv("../tidy_data/DETECTR_042_tidy_metadata.csv")
standard_041 <- subset(DETECTR_042, DETECTR_042$sample_set == "AMP_standard" & !(is.na(DETECTR_042$time)) & !(is.na(DETECTR_042$rfu_value)))
standard_041$standard_conc <- as.numeric(as.character(standard_041$standard_conc))
...
samples_040 <- subset(DETECTR_042, DETECTR_042$time=="00:50:00" & DETECTR_042$sample_set=="AMP_0_proximal" )
...
samples_040 <- subset(DETECTR_042, DETECTR_042$time=="00:50:00" & DETECTR_042$sample_set=="UMP_water_proximal" )
...
samples_040 <- subset(DETECTR_042, DETECTR_042$time=="00:50:00" & DETECTR_042$sample_set %in% c("UMP_water_proximal", "AMP_0_proximal","AMP_0_distal"))
...
samples_040 <- subset(DETECTR_042, DETECTR_042$time=="00:50:00" & DETECTR_042$kit %in% c("water"))
...
DETECTR_042 <- subset(DETECTR_042,  DETECTR_042$standard_conc < 0.2)
